...
load("/mmfs1/home/rzs0112/code_working_folder/general functions/NN-test_quantile/NN-test-simul_10d_999%.RData")
setwd("/mmfs1/home/rzs0112/code_working_folder/CCDs_Clustering/UN_CCDs/Simulations/Gaussian/10d")
library(mclust)
library(parallel)
library(doParallel)
library(MASS)
...
# generate data sets
data_list = lapply(1:iteN, Gaussian_cls, n_cls=n_cls, 
                   d=d, 
                   centers=MU,
                   sigma=sig,
                   noise=F,
...
clustering_result = foreach(x=data_list,.packages = c("MASS","cluster")) %dopar% UNMCCD_outlier(datax=x, simul=simul, method=method)
stopCluster(cl)
...
print(paste("UN-CCDs: The mean ARI is", mean(ARI_2cls),",",
...
print(paste("UN-CCDs: The mean ARI is", mean(ARI_3cls),",",
...
print(paste("UN-CCDs: The mean ARI is", mean(ARI_5cls),",",
...
save.image("10d_235clusters_n200.RData")