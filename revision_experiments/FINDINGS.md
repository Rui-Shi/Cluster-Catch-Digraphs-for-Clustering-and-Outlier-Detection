...
**Scope decision (user, 2026-07-16): Speech (3686×400) excluded from WP5 CCD rows.** The NN table covers cluster sizes only up to 1000; beyond that the harness clamps to the last table entry (harness.R:153), so most of Speech's inlier mass would be tested against wrong critical values. Same exposure exists for Musk (3062×166) — pending user call; Arrhythmia (452×274) is clean (n<1000, no clamp).
