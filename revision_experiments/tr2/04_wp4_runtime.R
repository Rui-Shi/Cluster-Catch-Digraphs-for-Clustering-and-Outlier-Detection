#!/usr/bin/env Rscript
# revision_experiments/tr2/04_wp4_runtime.R
#
...
# unchanged. UNCCD at d = 500 DOES run: revision_experiments/tr2/01i_nn_d500_
# quant.R (written, not run by this file) generates the missing NN table.
#
# USAGE
#   Rscript "revision_experiments/tr2/04_wp4_runtime.R" <reps> <mode> [cores]
#
...
# 01i_nn_d500_quant.R before the production run, see that file's header).
...
# revision_experiments/tr2/07b_wp5_fulldata_ccd.R's validated parallel radius
...
if (length(args) < 2) {
  stop("Usage: Rscript \"revision_experiments/tr2/04_wp4_runtime.R\" <reps> <mode: all|r_only|cell=GRID:VALUE|micro> [cores]",
       call. = FALSE)
}
...
# ---------------------------------------------------------------------------
# Ported from revision_experiments/tr2/07b_wp5_fulldata_ccd.R (read-only source;
...
nnccd.radi.serial <- nnccd.radi   # harness.R's clamped serial version; kept
                                   ...
                                   # (revision_experiments/tr2/04b_validate_parallel_radii.R)
...
nnccd.radi <- function(dx, quantile = "lower", method = "ascend", low.num, quant,
  ...
  # this strip is required for bit-exactness (see 04b_validate_parallel_radii.R).
