...
nnccd_clustering_quantile <- function(datax,low.num=3, quantile="lower", 
                                      method="accend", dom.method="greedy2", quant=0.90, simul=NULL, niter=100){
  # info
  nr <- nrow(datax)
  nc <- ncol(datax)
  ...
  ccd.info <- nnccd.radi(dx=datax, quantile, method, low.num, quant, simul=simul, niter)
...
nnccd.silhouette <- function(graph,datax,cls=NULL,ind=NULL, lenDlimit=Inf){
  ddatax = as.matrix(dist(datax))
...
nnccd.radi <- function(dx, quantile="lower", method="accend", low.num, quant, simul=NULL, niter){
  ...
  if(quantile=="lower"){
    ...
    else {NN.envelop <- NNDest.simpois.lower.quant(n, d, (1+quant)/2, niter)}
    ...
    for(i in 1:n){
      if(method == "accend"){
        ...
        for(j in low.num:n){
          ...
          upper.bound.var = NN.envelop$variance[j-1]
          # if(NN.dist.obs$averge<lower.bound.ave | NN.dist.obs$variance<lower.bound.var){
          #   R[i] = ddx[i,o.d[j-1]]
          #   break
          # }
          if(NN.dist.obs$averge<lower.bound.ave | NN.dist.obs$variance>upper.bound.var){
            if(j == low.num) R[i] = 0
      ...
      if(method=="decend"){
        ...
        for(j in 1:(n-low.num)){
          ...
          upper.bound.var = rev(NN.envelop$variance)[j+2]
          if(NN.dist.obs$averge>lower.bound.ave & NN.dist.obs$variance<upper.bound.var){
            R[i] = r
            break
          }
