...
source("/media/rui/exNVME/code_working_folder/Outlyingness_Score/UNCCD_OOS_IOS.R")
...
source("/media/rui/exNVME/code_working_folder/UNCCD_OOS_IOS/Simulation/Gaussian/Threshold.R")
library(parallel)
library(doParallel)
library(MASS)
library(igraph)
...
OOS_scores = foreach(x=data.list,.packages = c("MASS","cluster","igraph")) %dopar% UNCCD_OOS(datax=x,simul=simul,method=method,d=d)
IOS_scores = foreach(x=data.list,.packages = c("MASS","cluster","igraph")) %dopar% UNCCD_IOS(datax=x,simul=simul,method=method,d=d)
stopCluster(cl)
...
covM2 = diag(1,d)
covM2[lower.tri(covM2)] = -(rho-0.2)
covM2[upper.tri(covM2)] = -(rho-0.2)

covM3 = diag(1,d)
covM3[lower.tri(covM3)] = rho+0.2
covM3[upper.tri(covM3)] = rho+0.2
