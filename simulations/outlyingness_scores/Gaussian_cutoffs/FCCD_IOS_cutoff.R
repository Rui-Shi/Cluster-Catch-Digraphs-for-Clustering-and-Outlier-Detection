...
cutoff_IOS = function(x, simul, d, n, n0, quant){
  scores = sort(FCCD_IOS(datax = x, simul=simul, d=d, quant=quant))
...
FCCD_IOS_2d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,simul=simul,d=d,n=n,n0=n0,quant=quant)
stopCluster(cl)

median(unlist(FCCD_IOS_2d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FCCD_IOS_3d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,simul=simul,d=d,n=n,n0=n0,quant=quant)
stopCluster(cl)

median(unlist(FCCD_IOS_3d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FCCD_IOS_5d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,simul=simul,d=d,n=n,n0=n0,quant=quant)
stopCluster(cl)

median(unlist(FCCD_IOS_5d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FCCD_IOS_10d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,simul=simul,d=d,n=n,n0=n0,quant=quant)
stopCluster(cl)

median(unlist(FCCD_IOS_10d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FCCD_IOS_20d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,simul=simul,d=d,n=n,n0=n0,quant=quant)
stopCluster(cl)

median(unlist(FCCD_IOS_20d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FCCD_IOS_50d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,simul=simul,d=d,n=n,n0=n0,quant=quant)
stopCluster(cl)

median(unlist(FCCD_IOS_50d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FCCD_IOS_100d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,simul=simul,d=d,n=n,n0=n0,quant=quant)
stopCluster(cl)

median(unlist(FCCD_IOS_100d))

save.image("/media/rui/exNVME/code_working_folder/Outlyingness_Score/FCCD_IOS_cutoff.RData")
t2 = Sys.time()
t2-t1