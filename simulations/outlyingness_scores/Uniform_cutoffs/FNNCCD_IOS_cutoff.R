...
cutoff_IOS = function(x, method="ascend", simul, d, n, n0){
  scores = sort(FNNCCD_IOS(datax = x, method=method, simul=simul, d=d))
...
FNNCCD_IOS_2d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,simul=simul,d=d,n=n,n0=n0)
stopCluster(cl)

median(unlist(FNNCCD_IOS_2d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FNNCCD_IOS_3d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,simul=simul,d=d,n=n,n0=n0)
stopCluster(cl)

median(unlist(FNNCCD_IOS_3d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FNNCCD_IOS_5d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,simul=simul,d=d,n=n,n0=n0)
stopCluster(cl)

median(unlist(FNNCCD_IOS_5d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FNNCCD_IOS_10d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,method="descend",simul=simul,d=d,n=n,n0=n0)
stopCluster(cl)

median(unlist(FNNCCD_IOS_10d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FNNCCD_IOS_20d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,method="descend",simul=simul,d=d,n=n,n0=n0)
stopCluster(cl)

median(unlist(FNNCCD_IOS_20d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FNNCCD_IOS_50d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,method="descend",simul=simul,d=d,n=n,n0=n0)
stopCluster(cl)

median(unlist(FNNCCD_IOS_50d))
...
cl <- makeCluster(cores)
registerDoParallel(cl)
FNNCCD_IOS_100d = foreach(x=data.list, .packages = c("MASS","cluster")) %dopar% cutoff_IOS(x,method="descend",simul=simul,d=d,n=n,n0=n0)
stopCluster(cl)
