...
labels_list = foreach(x=data.list,.packages = c("MASS","cluster","FNN")) %dopar% ODIN(x)
stopCluster(cl)

count.result = foreach(x=1:iteN,.combine = rbind) %do% count_ODIN2(data.num[[x]][1], data.num[[x]][2], labels_list[[x]])
mean = c(mean(count.result[,1]),mean(count.result[,2]),mean(count.result[,3]),mean(count.result[,4]))

print(paste("ODIN: the mean TPR is", mean[1],",","the mean TNR", mean[2],",","the mean BA", mean[3],",","the mean F2", mean[4]))
