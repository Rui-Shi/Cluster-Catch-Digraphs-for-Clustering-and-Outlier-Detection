...
# use MST for clustering and outlier detection
# data: the input data set
# cont: the minimal % of a cluster
# thresh: the ratio to cut a edge when comparing its adjacent edges
MST_Outlier = function(data, cont=0.04, thresh=2){
  n = dim(data)[1]
  ...
  cls[cls %in% names(size_cls)[size_cls < n*cont]] = 0 # label outliers as 0
  return(labels = cls)
}
...
count_MST2 = function(n1, n0, es_labels){
  n = n1 + n0
