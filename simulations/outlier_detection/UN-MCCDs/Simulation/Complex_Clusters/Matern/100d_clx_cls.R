...
NNCCD.outlier = function(datax, simul){
  set.seed(321)
...
outlier.result = foreach(x=data.list,.packages = c("MASS","cluster","igraph")) %dopar% NNCCD.outlier(x, simul)
stopCluster(cl)
t2 = Sys.time()
t2-t1
...
save.image(here::here("simulations/outlier_detection/UN-MCCDs/Simulation/Complex_Clusters/Matern/100d_clx_cls.RData"))
tt2 = Sys.time()
tt2-tt1