...
load("/mmfs1/home/rzs0112/code_working_folder/general functions/RK-test_quantile/RK-test-simul_10d_99%.RData")
library(parallel)
library(doParallel)
library(MASS)
library(igraph)
...
MFCCD.outlier = function(datax, simul){
  ...
  result.cls = rccd.silhouette_mutual(Mgraph,datax,ind=NULL, lenClimit=Inf,k=5, min.cls = max(0.04,cont/2))
