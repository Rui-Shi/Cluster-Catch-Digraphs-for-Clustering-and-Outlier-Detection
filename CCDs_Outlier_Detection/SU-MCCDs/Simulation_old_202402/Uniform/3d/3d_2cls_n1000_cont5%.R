...
count = function(x){
  ...
  outlier.true = data.list[[x]][-c(1:(n-n0)),] # true outliers
  outlier.detect = NULL # outliers captured
  for(i in 1:clsNum){
    mode = which.max(table(outlier.result[[x]]$label[[i]]))
    outlier.temp = outlier.result[[x]]$clusters[[i]][outlier.result[[x]]$label[[i]]!=mode,]
    outlier.detect = rbind(outlier.detect,outlier.temp)
  }
  success.rate = length(intersect(outlier.true[,1], outlier.detect[,1]))/n0
  false.positive = max((nrow(outlier.detect)-success.rate*n0)/(n-n0),0)
  return(c(success.rate=success.rate,false.positive=false.positive))
}
