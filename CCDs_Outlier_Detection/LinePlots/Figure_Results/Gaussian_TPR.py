...
file_path = 'G:/code_working_folder/Simulation_Results/Figure_Results/Gaussian_TPR_TNR.xlsx'
...
# Populating the dictionary correctly
for col in range(2, data.shape[1], 2):  # Start from the second column and step by 2 to skip TNR columns
    size = data.iloc[0, col]  # Correctly retrieve the data size from the first row
    for row in range(2, data.shape[0]):
        if pd.notna(data.iloc[row, 0]):
            current_dimension = data.iloc[row, 0].split('=')[
                1].strip()  # Update current dimension when a new one is found
        ...
        plot_data['TPR'].append(float(data.iloc[row, col]))
...
pivot_df = plot_df.pivot_table(index=['dimension', 'algorithm'], columns='data_size', values='TPR',
...
plt.suptitle('TPRs with Gaussian Clusters', fontsize=16)
