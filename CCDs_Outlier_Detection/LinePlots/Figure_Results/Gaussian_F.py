...
for col in range(3, data.shape[1], 2):  # Start from the third column and step by 2 to skip TPR columns
    ...
    for row in range(2, data.shape[0]):
        ...
        plot_data['F-score'].append(float(data.iloc[row, col]))
...
pivot_df = plot_df.pivot_table(index=['dimension', 'algorithm'], columns='data_size', values='F-score', aggfunc='first').reset_index()
...
for ax, algorithm in zip(axes, algorithm_order):
    ...
    ax.set_ylabel('$F_2$-score', fontsize=16)
    ax.legend(title='Data Size $n$')

plt.suptitle('$F_2$-scores with Gaussian Clusters', fontsize=16)
