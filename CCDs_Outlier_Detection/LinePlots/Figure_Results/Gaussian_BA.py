...
file_path = 'G:/code_working_folder/Simulation_Results/Figure_Results/Gaussian_BA_F.xlsx'
...
for col in range(2, data.shape[1], 2):  # Start from the second column and step by 2 to skip TNR columns
    ...
    for row in range(2, data.shape[0]):
        ...
        plot_data['BA'].append(float(data.iloc[row, col]))
...
pivot_df = plot_df.pivot_table(index=['dimension', 'algorithm'], columns='data_size', values='BA',
...
plt.suptitle('BAs with Gaussian Clusters', fontsize=16)
