...
MFNNCCD_outlier = function(datax, simul=NULL, min.cls=0, method="ascend", low.num=3, k=NULL){
  # datax = data.list[[1]]
  # simul=simul; min.cls=0.04; method="ascend"; low.num=3; k=NULL
  n = dim(datax)[1]
  d = dim(datax)[2]
  ...
  dom_index = NN_result$Int.D # the index of the dominating covering balls
  ...
  graph_full = graph
  graph_full$member=nnccd.clustering.nonvalid.knn1(graph, datax, k)
  ...
  result_cls = nnccd.silhouette_mutual1(graph=graph_full, datax=datax, k=k, min.cls = min.cls)
